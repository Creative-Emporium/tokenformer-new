...
class Dataset:
    ...
    def __init__(self, config, language, elements=100):
        # TODO: handle multiple languages
        with open(os.devnull, 'w') as f, redirect_stderr(f):
            dataset = load_dataset('uonlp/CulturaX', language,
    ...
    def evaluate_perplexity(self, model, mask=None):
        ...
        for i in tqdm.trange(0, len(tokens[0]) - self.context, self.context,
                             desc=f'\tEvaluating perplexity [{self.language}]',
                             ncols=100):
